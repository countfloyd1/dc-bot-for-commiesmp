...
async def send_to_gulag(message, word, gulag_messages):
    try:
    ...
    try:
        await message.author.timeout(
            ...
            reason=f"Counter-revolutionary speech: '{word}'"
        )
    except discord.Forbidden:
        pass
...
async def on_message(message):
    ...
    # Check for communist words and whether used positively or negatively
    found_praise = None
    found_anti = None
    ...
    # Gulag for anti-communist speech (admins exempt)
    if found_anti and not is_admin:
        await send_to_gulag(message, found_anti, ANTI_COMMUNIST_GULAG_MESSAGES)
        await bot.process_commands(message)
        return

    # Gulag for capitalist words (admins exempt)
    if not is_admin:
        found_word = None
        ...
        if found_word:
            await send_to_gulag(message, found_word, GULAG_MESSAGES)
